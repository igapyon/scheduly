...
- Webpack 版は `npm run dev` でホットリロードしながら作業します。エントリポイントは管理者向け `src/frontend/admin.jsx`（`public/index.html`）、参加者入力向け `src/frontend/user.jsx`（`public/user-edit.html`）、参加者回答一覧 `src/frontend/admin-responses.jsx`（`public/user.html`）。
...
- `src/frontend/` に React エントリ（管理者向け `admin.jsx` → `index.bundle.js`、参加者入力向け `user.jsx` → `user.bundle.js`、参加者回答一覧 `admin-responses.jsx` → `responses.bundle.js`）を配置し、スタイルは当面 HTML 側で読み込む Tailwind CDN と最小限のインライン CSS で対応。
- `public/index.html`（管理画面） / `public/user-edit.html`（参加者入力画面） / `public/user.html`（参加者回答一覧）で Tailwind CDN を読み込みます。管理画面では ical.js CDN も読み込んでいます。
- 開発時は `npm run dev` で `webpack-dev-server`（ポート 5173）を起動。`http://localhost:5173/index.html`（管理者）、`http://localhost:5173/user.html`（参加者回答一覧）、`http://localhost:5173/user-edit.html`（参加者入力画面）を必要に応じて開く。Console を必ず確認。
