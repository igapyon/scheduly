...
- ゴール: 参加者一覧 (`user.jsx`) の「回答」操作を遷移せず完結させるため、1名・1日程分のみ展開するインライン編集コンポーネント（○△× とコメント欄）を参加者カードに埋め込む。
- UI 方針
  - 「回答」ボタン押下で該当カード内に編集 UI を挿入し、ほかのカードは自動で閉じるトグル制御にする。開いているカードへの再押下で閉じられるようにする。
...
- 移行ステップ（案）
  1. 編集コンポーネントの切り出しとサービス層の共通化。
  2. 参加者カードへの組み込みと UX 検証（デスクトップ／モバイル）。
  3. 安定後、旧「回答編集」画面の導線を整理し、最終的な削除タイミングを判断する（確信が持てるまで併存）。
