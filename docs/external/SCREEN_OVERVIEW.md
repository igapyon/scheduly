...
Scheduly の UI は次の 2 画面で構成されます。ここでは「何ができるか」を中心に、役割と主要要素、画面間の関係を簡潔に整理します。アクセス方式や保存ポリシーなどの前提は `docs/external/EXTERNAL_ASSUMPTIONS.md` を参照。技術的な詳細は `docs/internal/SCREEN_TECHNICAL_OVERVIEW.md` を参照してください。
...
保存方針やバリデーションの前提は `docs/external/EXTERNAL_ASSUMPTIONS.md` を参照。技術的なデータフローや再計算の仕組みは `docs/internal/SCREEN_TECHNICAL_OVERVIEW.md` に記載しています。
