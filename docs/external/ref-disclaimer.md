...
- 本ソフトウェアはブラウザの `localStorage`（利用できない環境では `sessionStorage`）にキャッシュを保持しつつ、API サーバー側に正規データを保存します。ルートアクセスごとに新しい `projectId` が払い出され、共有 URL を発行しない限り他セッションに同じデータが見えることはありません。とはいえ、個人情報や機微情報を扱う際は利用者側で適切な永続ストア・暗号化・アクセス制御を設計してください。
