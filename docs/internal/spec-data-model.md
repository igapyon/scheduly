...
- **保管期間**
  - クライアントは localStorage（利用できない場合は sessionStorage）へキャッシュを保持するが、正規データは API サーバーの `ProjectState` に保存される。ルートアクセス時は毎回新しい `projectId` が払い出され、URL トークンを知らない限り他セッションと共有されない。
