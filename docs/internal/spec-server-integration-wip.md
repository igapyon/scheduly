...
- これまでは「単一の `demo-project` をブラウザの sessionStorage で共有する」前提だったが、今後は **プロジェクト ID ごとに完全に独立した shareTokens/state** を管理する。本番ホスティングを想定した際に、別セッションで以前の管理 URL が表示される挙動は NG。
